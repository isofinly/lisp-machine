...
class InvalidArgTypeError(Exception):
    def __init__(self, arg_type: Any) -> None:
...
class DataPath:
    ...
    def latch_dr(self, signal: DrSelSignal, alu_res: int | None = None, mem_value: int | None = None) -> None:
...
class ControlUnit:
    ...
    def __repr__(self) -> str:
        ...
        return (
            f"INSTR_N: {self.executed_instruction_n}, "
...
def find_next_stage_from_instruction_fetch(control_unit: ControlUnit, decoded_instruction: Instruction) -> None:
...
def handle_instruction_fetch_tick(control_unit: ControlUnit) -> None:
    if control_unit.tc == 0:
        ...
        control_unit.decoded_instruction = result
        ...
        decoded_instruction: Instruction = control_unit.decoded_instruction
        ...
        decoded_instruction = control_unit.decoded_instruction
...
def find_next_stage_after_address_fetch(control_unit: ControlUnit, decoded_instruction: Instruction) -> None:
...
def handle_address_fetch_tick(control_unit: ControlUnit) -> None:
    if control_unit.decoded_instruction is None:
        raise UncodedInstructionError(control_unit)
    decoded_instruction: Instruction = control_unit.decoded_instruction
    ...
    if decoded_instruction.arg is None:
        raise UncodedInstructionError(control_unit)
    arg: Arg = decoded_instruction.arg
...
def handle_operand_fetch_tick(control_unit: ControlUnit) -> None:
    value = control_unit.data_path.oe(AddrSelSignal.DR)
    if not isinstance(value, int):
        raise InvalidValueTypeFromMemoryError(value)
...
def command_handle_execute_ld(control_unit: ControlUnit) -> None:
    control_unit.data_path.latch_ac(AcSelSignal.DR)
    control_unit.stage = Stage.INSTRUCTION_FETCH


def command_handle_execute_st(control_unit: ControlUnit) -> None:
    control_unit.data_path.wr(AddrSelSignal.DR)
    control_unit.stage = Stage.INSTRUCTION_FETCH


def command_handle_execute_binop(control_unit: ControlUnit, op: AluOp) -> None:
...
def command_handle_execute_jz(control_unit: ControlUnit) -> None:
...
def command_handle_execute_jnz(control_unit: ControlUnit) -> None:
...
def command_handle_execute_jmp(control_unit: ControlUnit) -> None:
    control_unit.data_path.latch_ip(IpSelSignal.DR)

    control_unit.stage = Stage.INSTRUCTION_FETCH


def command_handle_execute_push(control_unit: ControlUnit) -> None:
    if control_unit.tc == 0:
...
def command_handle_execute_pop(control_unit: ControlUnit) -> None:
    control_unit.data_path.latch_sp(SpSelSignal.INC)
    control_unit.stage = Stage.INSTRUCTION_FETCH


def command_handle_execute_in(control_unit: ControlUnit) -> None:
    control_unit.data_path.latch_ac(AcSelSignal.IN)
    control_unit.stage = Stage.INSTRUCTION_FETCH


def command_handle_execute_out(control_unit: ControlUnit) -> None:
    control_unit.data_path.latch_out()
    control_unit.stage = Stage.INSTRUCTION_FETCH


def command_handle_execute_call(control_unit: ControlUnit) -> None:
    if control_unit.tc == 0:
        control_unit.data_path.latch_ip(IpSelSignal.INC)
        control_unit.data_path.latch_sp(SpSelSignal.DEC)
...
def command_handle_execute_ret(control_unit: ControlUnit) -> None:
    if control_unit.tc == 0:
...
def command_handle_execute_hlt(control_unit: ControlUnit) -> None:
...
def handle_execute_tick(control_unit: ControlUnit) -> None:
    if control_unit.decoded_instruction is None:
        raise UncodedInstructionError(control_unit)
...
def handle_tick(control_unit: ControlUnit) -> None:
