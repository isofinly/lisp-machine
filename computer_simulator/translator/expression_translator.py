...
class Program:
    ...
    def to_machine_code(self) -> bytes:
        ...
        # Insert JMP instruction at address 0 to jump to address 512
        binary_code.extend((int(Opcode.JMP.binary, 2) << 24).to_bytes(4, byteorder="big"))
        binary_code.extend((512).to_bytes(4, byteorder="big"))

        for i, instruction in enumerate(self.memory):
            if i < 512:
                continue
            if isinstance(instruction, Instruction):
                opcode_binary = int(instruction.opcode.binary, 2) << 24
                if instruction.arg:
                    addressing_type = {
                        ArgType.DIRECT: 0b00 << 22,
                        ArgType.ADDRESS: 0b01 << 22,
                        ArgType.INDIRECT: 0b10 << 22,
                        ArgType.STACK_OFFSET: 0b11 << 22,
                    }[instruction.arg.arg_type]
                    opcode_binary |= addressing_type
                binary_code.extend(opcode_binary.to_bytes(4, byteorder="big"))
                if instruction.arg:
                    if instruction.arg.value is not None:
                        binary_code.extend(instruction.arg.value.to_bytes(4, byteorder="big"))
                else:
                    binary_code.extend((0).to_bytes(4, byteorder="big"))
...
def increment_index(result: Program, index_name: str = "#i") -> None:
