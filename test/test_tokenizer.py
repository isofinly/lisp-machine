...
class TestTokenize:
    def test_whitespace_processing_handles_empty_input(self) -> None:
        result = tokenize("")
        assert result == []

    def test_whitespace_processing_handles_only_whitespace(self) -> None:
        result = tokenize("   \n\t  ")
        assert result == []

    def test_number_literal_processing_handles_non_numeric_input(self) -> None:
        result = tokenize("abc")
        assert result == [Token(Token.Type.IDENTIFIER, "abc")]

    def test_identifier_processing_handles_non_alpha_input(self) -> None:
        result = tokenize("123")
        assert result == [Token(Token.Type.INT, "123")]

    def test_string_literal_processing_handles_unterminated_string(self) -> None:
        with pytest.raises(InvalidSymbolsError):
            tokenize('"abc')

    def test_tokenize_handles_complex_input(self) -> None:
    ...
    def test_tokenize_handles_complex_input2(self) -> None:
        result = tokenize("(+ (1) (2))")
        assert result == [
            Token(Token.Type.OPEN_BRACKET, "("),
            Token(Token.Type.BINOP, "+"),
