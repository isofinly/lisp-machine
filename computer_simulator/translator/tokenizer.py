...
def process_brackets(tokens: list[Token], idx: int, chars: str) -> int:
    if idx >= len(chars):
        return idx
    if chars[idx] == "(":
...
def process_binpos(tokens: list[Token], idx: int, chars: str) -> int:
    if idx >= len(chars):
        return idx
    if chars[idx] in ("+", "-", "*", "/", "=", "<", ">", "%"):
...
def process_number_literal(tokens: list[Token], idx: int, chars: str) -> int:
    if idx >= len(chars):
        return idx
    start_idx = idx
...
def process_identifier(tokens: list[Token], idx: int, chars: str) -> int:
    if idx >= len(chars):
        return idx
    start_idx = idx
...
def process_string_literal(tokens: list[Token], idx: int, chars: str) -> int:
    if idx >= len(chars):
        return idx
    if chars[idx] == '"':
        value = ""
...
def process_booleans(tokens: list[Token], idx: int, chars: str) -> int:
    if idx >= len(chars):
        return idx
    if chars[idx] in ("T", "F"):
...
def process_keyword(tokens: list[Token], idx: int, chars: str, token_type: Token.Type, keyword: str) -> int:
    if idx >= len(chars):
        return idx
    value = ""
    end_idx = idx
